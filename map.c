...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    { // Step 2: 输出结果并清理
        ...
        for (int i = 0; i < s->n_seq; ++i)
        {
            ...
            trf_print_bed(s->myResult[i].GlobalIndexList, &s->seq[i], stdout);
            if (p->opt->datafile)
            {
                print_Data_File(s->myResult[i], &s->seq[i], p->opt, p->datafile_fp, i);
            }

            if (p->opt->ngs)
            {
                print_NGS_File(s->myResult[i], &s->seq[i], p->ngsfile_fp);
            }

            if (p->opt->masked)
            {
                print_Mask_File(s->myResult[i], &s->seq[i], p->masked_fp);
            }
...
int trf_search_file(const char *fn, const trf_opt *opt, long tbatch_size, readonly_vars_struct *ro_vars)
{
    ...
    if (opt->masked) //
    {
        pl.masked_fp = fopen(opt->maskfilename, "w");
    ...
    if (opt->datafile) //
    {
        pl.datafile_fp = fopen(opt->datafilename, "w");
    ...
    if (opt->ngs) //
    {
        pl.ngsfile_fp = fopen(opt->ngsfilename, "w");
    ...
    // 确定流水线阶段数
    const int n_stages = (opt->n_threads == 1) ? 1 : 2;
    ...
    if (opt->datafile) //
    {
        fclose(pl.datafile_fp);
    }

    if (opt->ngs) //
    {
        fclose(pl.ngsfile_fp);
    }
